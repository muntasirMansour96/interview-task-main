...
public class NormalizerTest {
    ...
    public void givenInvalidInput_whenMinMaxScale_thenThrowException() throws IOException {
        ...
        exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.minMaxScaling(source,
                        Files.createTempFile("target", ".csv"), "" +
                                "Kalven"));
    ...
    @Test
    public void givenInvalidNormalizeColum_whenZscore_thenThrowException() throws IOException {
        Normalizer normalizer = normalizer();
        Path source = copyFile("/marks.csv", Files.createTempFile("marks", ".csv"));
        IllegalArgumentException  exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.zscore(source,
                        Files.createTempFile("target", ".csv"), "name"));
        Assertions.assertEquals("cannot make normalize to colum name", exception.getMessage());

    }

    @Test
    public void givenInvalidNormalizeColum_whenMinMaxScaled_thenThrowException() throws IOException {
        Normalizer normalizer = normalizer();
        Path source = copyFile("/marks.csv", Files.createTempFile("marks", ".csv"));
        IllegalArgumentException  exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.minMaxScaling(source,
    ...
    private Path copyFile(String resource, Path path) throws IOException {
    ...
    @Test
    public void givenInvalidInputDestinationPathDirectory_whenZscore_thenThrowException() throws IOException {
        String filename = "employees.csv";
        Path induction = Files.createTempDirectory("induction");
        String columnName = "salary";
        Path csvPath = induction.resolve(filename);
        ...
        Normalizer normalizer = normalizer();
        IllegalArgumentException exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.zscore(csvPath, Paths.get("/no_exist/name.csv"), columnName));
        Assertions.assertEquals("Destination directory not found", exception.getMessage());
    }

    @Test
    public void givenInvalidInputDestinationPathDirectory_whenMinMaxScale_thenThrowException() throws IOException {
        String filename = "employees.csv";
        Path induction = Files.createTempDirectory("induction");
        String columnName = "salary";
        Path csvPath = induction.resolve(filename);
        ...
        Normalizer normalizer = normalizer();
        IllegalArgumentException exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.minMaxScaling(csvPath, Paths.get("/no_exist/name.csv"), columnName));
        Assertions.assertEquals("Destination directory not found", exception.getMessage());
    }

    @Test
    public void givenInvalidInputDestinationFileTypeNotCsv_whenZscore_thenThrowException() throws IOException {
        String filename = "employees.csv";
        Path induction = Files.createTempDirectory("induction");
        String columnName = "salary";
        Path csvPath = induction.resolve(filename);
        Path destPath = induction.resolve("employees_scaled.exe");
        copyFile("/employees.csv", csvPath);
        Assertions.assertTrue(Files.exists(csvPath));

        Normalizer normalizer = normalizer();
        IllegalArgumentException exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.zscore(csvPath, destPath, columnName));
        Assertions.assertEquals("Destination file not csv exception", exception.getMessage());

        exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.zscore(csvPath, induction.resolve(".csv"), columnName));
        Assertions.assertEquals("Destination file not csv exception", exception.getMessage());

        exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.zscore(csvPath, induction.resolve("name.exe.csv"), columnName));
        Assertions.assertEquals("Destination file not csv exception", exception.getMessage());
    ...
    @Test
    public void givenInvalidInputDestinationFileTypeNotCsv_whenMin_Max_Scale_thenThrowException() throws IOException {
        String filename = "employees.csv";
        Path induction = Files.createTempDirectory("induction");
        String columnName = "salary";
        Path csvPath = induction.resolve(filename);
        Path destPath = induction.resolve("employees_scaled.txt");
        copyFile("/employees.csv", csvPath);
        Assertions.assertTrue(Files.exists(csvPath));

        Normalizer normalizer = normalizer();
        IllegalArgumentException exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.minMaxScaling(csvPath, destPath, columnName));
        Assertions.assertEquals("Destination file not csv exception", exception.getMessage());

        exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.minMaxScaling(csvPath, induction.resolve(".csv"), columnName));
        Assertions.assertEquals("Destination file not csv exception", exception.getMessage());

        exception = Assertions.assertThrows(IllegalArgumentException.class,
                () -> normalizer.minMaxScaling(csvPath, induction.resolve("name.exe.csv"), columnName));
        Assertions.assertEquals("Destination file not csv exception", exception.getMessage());
